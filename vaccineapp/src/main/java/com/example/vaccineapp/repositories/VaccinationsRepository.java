...
public interface VaccinationsRepository extends JpaRepository<Vaccinations, Long> {
	...
	List<Vaccinations> findByCitizenId(Long citizenId);

	@Query("""
			    SELECT vh
			    FROM Vaccinations vh
			    WHERE LOWER(vh.citizen.fullName) LIKE LOWER(CONCAT('%', :name, '%'))
			      AND vh.citizen.phoneNumber LIKE CONCAT('%', :phone, '%')
			""")
	List<Vaccinations> findDetailedVaccinationHistory(@Param("name") String name, @Param("phone") String phone);
	...
	@Query("SELECT new com.example.vaccineapp.dto.VaccinationReportDTO(v.vaccinationDate, COUNT(v), "
			...
			+ "SUM(CASE WHEN v.status = 'MISSED' THEN 1 ELSE 0 END)) " + "FROM Vaccinations v "
	...
	@Query("SELECT new com.example.vaccineapp.dto.VaccinationRateByRegionDTO(w.name, COUNT(c), "
			...
			+ "FROM Citizen c LEFT JOIN Vaccinations vh ON c.id = vh.citizen.id "
